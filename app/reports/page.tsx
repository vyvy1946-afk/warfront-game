...
export default function ReportsPage() {
  ...
  const summary = useMemo(() => {
    ...
    const totalGold = reports.reduce((sum, report) => {
      return sum + (report.reward?.gold ?? 0)
    }, 0)

    const totalFuel = reports.reduce((sum, report) => {
      return sum + (report.reward?.fuel ?? 0)
    }, 0)

    const totalSteel = reports.reduce((sum, report) => {
      return sum + (report.reward?.steel ?? 0)
    }, 0)
      ...
      <section className="border-b border-zinc-800 bg-zinc-900/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black tracking-tight">보고서</h1>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <section className="mb-8 grid gap-4 md:grid-cols-5">
          ...
          <div className="rounded-3xl border border-yellow-500/20 bg-yellow-500/10 p-5">
            ...
            <p className="mt-2 text-3xl font-black">+{summary.totalGold}</p>
          </div>
            ...
            {reports.map((report) => {
              ...
              return (
                <article
                  key={report.id}
                  className={[
                    'rounded-3xl border p-6 shadow-2xl shadow-black/20',
                    isWin
